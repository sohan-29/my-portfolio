...
const AboutMe = () => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'overview':
        return <Overview aboutData={aboutData} />;
      case 'academics':
        return <Academics aboutData={aboutData} />;
      case 'experience':
        return <Experience aboutData={aboutData} />;
      case 'resume':
        return <Resume aboutData={aboutData} />;
      default:
        return <Overview aboutData={aboutData} />;
    }
  };
        ...
        <div role="tablist" className="tabs flex gap-4 overflow-x-auto no-scrollbar whitespace-nowrap">
          {tabs.map((tab) => (
            <button
              ...
              className={`tab tab-active flex items-center lg:text-base xl:text-lg md:text-xl hover:text-red-500 hover:underline transition-all duration-300 ${
                activeTab === tab.id
