      ...
      <div className="min-h-screen flex flex-col items-center bg-[#2b2b2b] text-white">
        ...
        <div className="relative flex items-center justify-center w-full">
          ...
          <div className="projectWindow flex w-[90%] mt-10 gap-7 overflow-x-scroll scrollbar-none py-4 cursor-pointer">
            {data.map(project => (
              <div key={project.id} onClick={() => setActiveProject(project.id)} className={`group h-57 min-w-44 aspect-auto sm:min-w-[27vw] sm:min-h-[33vh] bg-[url(${project.img})] bg-cover bg-center bg-no-repeat p-6 rounded-lg shadow-lg transition-shadow duration-300` + (activeProject === project.id ? " absolute top-7 w-[69vw] aspect-auto left-13.5 sm:left-[15%] min-w-[33vw] min-h-[44vh] overflow-y-scroll" : "")}>
